...
class FriendRepository(private val friendDao: FriendDao, private val apiService: MessengerAPIService) {

    private val friendRequests: LiveData<List<Friend>> = friendDao.getFriendsByFriendshipStatus(FriendshipStatus.Pending)
    private val friends: LiveData<List<Friend>> = friendDao.getFriendsByFriendshipStatus(FriendshipStatus.Friends)


    suspend fun sendFriendRequest(loggedInUser: Int, userNameOrEmail: String) :FriendRequestResponse{
        return apiService.sendFriendRequest(NewFriendRequest(
    ...
    suspend fun updateFriendshipStatus(selfUserId: Int, friendId: Int, friendUsername: String, friendshipStatus: FriendshipStatus):SuccessResponse{
